...
the recent Proposal "A B.N. for favor".</p>
<h4>Player Favor, Agora Nomic</h4>
<p>Introduction</p>
<p>Historically, many of the scams and CFJs in Agora Nomic
...
collecion of default assumptions, word-usage rules, and so on).</p>
<p>The Rules and Customs of Agora Nomic have historically
...
on it.</p>
<p>CFJs and scams based on word usage are welcomed and relished
...
be impossible in the first place.</p>
<p>It would be illuminating to consider what a Nomic game
...
as potentially leading to interesting games in its own right.</p>
<p>So</p>
<p>The idea is to design a completely formal Game, one in which
...
moves of, the game.</p>
<p>This will <em>not</em> be "real" Nomic unless we are wildly more
...
is intended to be a <em>different</em> game, albeit somewhat
...
<p>There will, of course, have to be an interface between
...
are <em>not</em> part of the game itself; any issues that come up
about the Mechanics ("I didn't make that move; someone must
have forged the e-mail!") will be extra-game issues, and
will not be expressed or resolved in the gamestate.</p>
<p>Similarly, the language in which the game is expressed will
...
cannot ultimately escape all bounds of language themselves.</p>
<p>Relationship to Agora Nomic</p>
<p>Formal Nomic can serve Agora Nomic as a thought-pump, if
...
<p>Formal Nomic might also be a useful, or instructive,
...
awarded the subgame's Treasury, or whatever.</p>
<p>Details / Examples</p>
<p>The rest of this paper consists of a worked-out example
...
and a considerable cost in difficulty of play.</p>
<p>For this example, the incoming interface is relatively
...
message.  For instance, a request to register might look like:</p>
<pre>
   From: Matt Zimola 
...
<p>and the corresponding object that gets created might be:</p>
<pre>
   objectId: 127
   batchId: 114
...
<p>"REGISTER MattZ" is syntactic sugar, to make a common operation
...
e wants to (except type, objectId, batchId, and so on).</p>
<p>On the other end, there will be a small set of verbs that
...
game-state itself, the sendFoo() verbs will be no-ops.</p>
<p>A rule is just another object, with Type "rule", an "if"
...
special effects.)</p>
<p>The sequence of events in the game is as follows.  Note that
...
enable laters efforts within the game.)</p>
<ul>
<li>
<p>Something happens: either a new object arrives from the
    physical world via the incoming interface, or a rule
    suddenly fires due to a schedule pop.</p>
</li>
<li>
<p>For each object of Type "rule" in the gamestate, in order
    ...
    (If a rule fires, but the "then" clause does <em>not</em> change
    the game-state, we don't restart.)</p>
</li>
<li>
<p>Once the highest-ruleOrder rule has had its "if" part
    evaluate to "false", we go to sleep again until the next
    thing happens.</p>
</li>
</ul>
<p>There are all sorts of "programming" issues involved in making these
...
the last player to submit a move wins".)</p>
<p>One way to manage this is to have a rule simply delete the object
...
to act on it as well.</p>
<p>Below is a sample Initial State.  It includes registration (but not
...
Agora concepts would fit into this set in reasonably simple ways.</p>
<p>Some other concepts are much harder; for instance, the idea
...
subject for further study.</p>
<p>I haven't actually <em>implemented</em> any of this as a computer program,
and I'm not sure that I have any interest in doing so.  But I hope
it will spur some interesting discussion nonetheless...</p>
<p>A note on scams: an amusing proof that scams are entirely
...
proposals.)</p>
<p>Anyway:</p>
<p>Basic attributes of objects:</p>
<pre>
...
<p>Move attributes set by the incoming interface (these attributes
are always assigned; any and all other attributes besides Type
can be set by the incoming mail explicitly):</p>
<pre>
...
<p>Except for these three, and type (which is always "move"), the
...
common cases.</p>
<p>Rule attributes used by the engine:</p>
<pre>
...
<p>Some functions for use in expressions:
  exists(match)
  count(match)
  timeGE(timevalue) - Tests if the current time is &gt;= the given
     ...
     at that time, if nothing else has happened in the meantime.</p>
<p>Notes:
  ...
  - Perhaps allow a rule to say that the engine <em>shouldn't</em> restart
  ...
  - Probably be good to have a setall(match1)(match2), that would
    efficiently set all objects matching match1 to match match2.</p>
<h2>An Intial State</h2>
<p>Loose conventions about ruleOrder:</p>
<pre>
...
<p>Initial engine settings and (empty) player-list:</p>
<pre>
  objectId: 1
  type: engineSettings
  runType: rule
...
<p>Implementing passed rulechanges:</p>
<pre>
  objectId: 24
  ruleOrder: 30520
  type: rule
